...
// State update fns
fn update_prey(s: &State<i32>, signals: &Signals<i32>) -> Update<i32> {
    let preys = s["preys"] + signals["preys_change"];
    Update { key: "preys", value: preys}
}

fn update_predator(s: &State<i32>, signals: &Signals<i32>) -> Update<i32> {
...
fn run_simulation() {
    ...
    let state_key_and_update_func_s: Vec<StateKeyAndUpdateFn<i32>> = vec![
